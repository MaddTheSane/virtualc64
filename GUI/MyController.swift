...
extension MyController {
    ...
    func processMessage(_ msg: vc64.Message) {
        ...
        */
        ...
        case .RS232_IN:
            var c = emu.userPort.rs232.readIncomingPrintableByte()
            while c != -1 {
                // serialIn += String(UnicodeScalar(UInt8(c)))
                // print("IN: \(UnicodeScalar(UInt8(c)))")
                c = emu.userPort.rs232.readIncomingPrintableByte()
            }

        case .RS232_OUT:
            var c = emu.userPort.rs232.readOutgoingPrintableByte()
            while c != -1 {
                // serialOut += String(UnicodeScalar(UInt8(c)))
                // print("OUT: \(UnicodeScalar(UInt8(c)))")
                c = emu.userPort.rs232.readOutgoingPrintableByte()
            }
