...
extension NSImage {
    ...
    static func make(data: UnsafeMutableRawPointer, rect: CGSize) -> NSImage? {
        ...
        let count = Int(rect.width) * Int(rect.height) * 4
        let dat2 = UnsafeMutableRawBufferPointer(start: data, count: count)
        guard let cgImage = CGImage.make(with: Data(dat2), size: rect) else {
